...
db.prediction_cache.createIndex({ "confidence": -1 }, { background: true });
db.prediction_cache.createIndex({ "isStale": 1, "lastUpdated": 1 }, { background: true });
...
db.push_subscriptions.createIndex({ "endpoint": 1 }, { unique: true, background: true });
db.push_subscriptions.createIndex({ "isActive": 1 }, { background: true });
db.push_subscriptions.createIndex({ "lastUsed": 1 }, { background: true });
...
db.users.createIndex({ "tier": 1 }, { background: true });
